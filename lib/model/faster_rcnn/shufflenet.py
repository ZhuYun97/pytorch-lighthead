  ...
  def __init__(self, num_classes=21, input_size=224, net_type=1):
    ...
    for stage_idx in range(len(self.stage_repeat_num)):
      ...
      for i in range(repeat_num):
        if i == 0:
          self.features.append(ShuffleBlock(in_c, out_c, downsample=True))
        else:
          self.features.append(ShuffleBlock(in_c, in_c, downsample=False))
        in_c = out_c
...
class shufflenetv2(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    # Build shufflenetv2
    self.RCNN_base = nn.Sequential(*(list(shufflenet.features._modules.values())))
